	...
	m_particles{ {-1.3f, -0.6f, -0.14f} }
{
	...
	sd2.AddressU = sd2.AddressV = D3D11_TEXTURE_ADDRESS_BORDER;
	sd2.BorderColor[3] = FLOAT(0);
	sd2.Filter = D3D11_FILTER_ANISOTROPIC;
	sd2.MaxAnisotropy = 16;
	// TODO : 1.10 Moddify MipLODBias field for the second sampler
	sd2.MipLODBias = 0;
	m_samplerMultipleTexture = m_device.CreateSamplerState(sd2);
	...
	XMStoreFloat4x4(&tempMtx,
		...
		* XMMatrixRotationZ(-XM_PI / 18)
		* XMMatrixScaling(1.f, -3/4.f, 1)
...
void RoomDemo::DrawWalls()
{
	...
	SetTextures({ m_wallTexture.get(), m_posterTexture.get()}, m_samplerMultipleTexture);
	...
	SetTextures({ m_wallTexture.get()}, m_samplerWrap);
	...
	XMStoreFloat4x4(&texMtx, 
		XMMatrixScaling(0.25, 4, 0)
		* XMMatrixTranslation(0.5, 8, 0));
	UpdateBuffer(m_cbTex1Mtx, texMtx);
